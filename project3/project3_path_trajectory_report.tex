...
此外，为了进一步检查规划器在不同障碍布局下的泛化能力，额外编写了
\nolinkurl{test_extra_path_cases.m}。该测试构造 5 个 SR3 自定义场景，包括无障碍基线、单球阻挡直线路径以及三球连续阻挡路径。测试结果如下：
...
自定义场景 & 直线路径 & 规划结果 & 说明 \\
\midrule
baseline & 可行 & 通过 & 返回 2 个路点 \\
high single & 不可行 & 通过 & 返回 3 个路点绕障 \\
sweep single & 不可行 & 通过 & 返回 3 个路点绕障 \\
diagonal single & 不可行 & 通过 & 返回 3 个路点绕障 \\
three spheres & 不可行 & 通过 & 返回 4 个路点绕过多球障碍 \\
\bottomrule
\end{tabular}
\end{table}
